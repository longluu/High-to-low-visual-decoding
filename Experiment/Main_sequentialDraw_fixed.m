function Main_sequentialDraw_fixed(params, fileLoad)
tStartExp = GetSecs;
...
if isempty(fileLoad)
    dataResponse = NaN(nTrials, 6);
    index = 1;
    for ii = 1:length(lineOrientation)
        for jj = 1:nTrialPerCondition
            if params.leftLine49 == 1
                dataResponse(index,1) = lineOrientation(1); 
                dataResponse(index,2) = lineOrientation(2);
            else
                dataResponse(index,1) = lineOrientation(2); 
                dataResponse(index,2) = lineOrientation(1);                
            end
            index = index + 1;
        end
    end
...
%                         eyeAcceleration(eyepos_SN) = velocityComputeOnline(eyeVelocity(2, eyepos_SN-4:eyepos_SN)');
    ...
    Screen('FillRect', windowDisplay, params.backgroundRGB);
    Screen('TextSize', windowDisplay, params.sizeTextPrompt);
    DrawFormattedText(windowDisplay, 'End of block 2. Thank you for your participation!', 'center', screenCenter(2)-params.yPositionPromptText, params.textColor);
    Screen('Flip', windowDisplay);  
    KbStrokeWait   
