function Main_separateDrawCenterFixate(params, fileLoad)
tStartExp = GetSecs;
...
%  Column 7: subject's categorical decision
%  Column 8: subject's reaction time categorical decision
nTrials = nTrialPerCondition * length(lineOrientation);
if isempty(fileLoad)
    dataResponse = NaN(nTrials, 8);
    index = 1;
...
%         mae_left_y_vali = sum(absoluteError_yvali_left) / validation.nPointValidation;
    ...
    rBigCircle = min(screenCenter) - lineDeviation;
    if isempty(fileLoad)
                        ...
                        if counterFixationInitial == params.nFixationSampleInitial
                            ...
                            if stimNotShown == 1
                                ...
                                Screen('DrawLines', windowDisplay, coordinateFirst,...
                                    params.lineWidthPixel, params.lineRGB, [screenCenter(1)-lineDeviation screenCenter(2)], 2);
                            ...
                            elseif GetSecs - tStartStim > params.lineDuration && counterClick < 2
                                % Time stamp for reaction time (first line)
                                if isStartEstimate == 1
                                    % Reset the position of prompt dot
                                    if isEstimateFirstLine
                                        % First line on the left
                                        screenCenter(1) = screenCenter(1) - lineDeviation;
                                    else
                                        % Second line on the right
                                        screenCenter(1) = screenCenter(1) + 2*lineDeviation;
                                    end
                                    ...
                                    Screen('DrawDots', windowDisplay,[screenCenter(1) screenCenter(2)], params.fixation(end), params.fixation(1:3), [], 2);                                      
                                    if isEstimateFirstLine
                                        Screen('DrawDots', windowDisplay,[screenCenter(1)+lineDeviation screenCenter(2)], params.fixation(end), params.fixation(1:3), [], 2); 
                                    else
                                        Screen('DrawDots', windowDisplay,[screenCenter(1)-lineDeviation screenCenter(2)], params.fixation(end), params.fixation(1:3), [], 2);                                         
                                    end
                                    Screen('Flip', windowDisplay);
                                    
                                    % Prompt subject to estimate the lines
                                ...
                                if any(buttonMouse)
                                    ...
                                    if counterClick == 1                                        
                                        % Save subject's estimate line 1
                                        ...
                                        % Set flag to change the pointer poistion
                                        isEstimateFirstLine = 0;
                                        isStartEstimate = 1;
                                        isMouseInitiated = 0;
                                    elseif counterClick == 2
                                        % Save subject's estimate line 2
                                        dataResponse(trialOrder(trialIndex),4) = rad2deg(thetaMouse);
                                        dataResponse(trialOrder(trialIndex),6) = GetSecs-tStartSecondEstimate;   
                                        
                                        % Set the center point to middle
                                        screenCenter(1) = screenCenter(1) - lineDeviation;
                                        
                                        % Draw the central fixation dot
                                        Screen('DrawDots', windowDisplay,[screenCenter(1) screenCenter(2)], params.fixation(end), params.fixation(1:3), [], 2);   
                                        Screen('Flip', windowDisplay, 1);                                        
                                    end
                                end
                            elseif counterClick == 2   
                                if isStartCatDecision
                                    % Prompt subject to give categorical
                                    % decision
                                    PsychPortAudio('FillBuffer', pahandleResponse, [responseBeep; responseBeep]);
                                    PsychPortAudio('Start', pahandleResponse, repetitions, startCue, waitForDeviceStart);

                                    % Get subject's categorical response (right
                                    % line is cw/ccw (1/2) relative to left line)
                                    startTimeEstimate = GetSecs;  
                                    isStartCatDecision = 0;
                                end

                                [keyIsDown, ~, keyCode, ~] = KbCheck;
                                if keyIsDown && ismember(find(keyCode), [KbName('LeftArrow'), KbName('RightArrow')])
                                    if find(keyCode) == KbName('LeftArrow') 
                                        subjResponse = 1; % ccw
                                    elseif find(keyCode) == KbName('RightArrow') 
                                        subjResponse = 2; % cw
                                    end
                                    dataResponse(trialOrder(trialIndex),7) = subjResponse;
                                    dataResponse(trialOrder(trialIndex),8) = GetSecs-startTimeEstimate;                            
                                    keepLooping = false;

                                    % Wait until key released
                                    KbReleaseWait;
                                elseif keyIsDown &&  find(keyCode)==KbName('q')
                                    % Wait until key released
                                    KbReleaseWait;
