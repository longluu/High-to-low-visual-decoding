function Train_ordinalJudgment(params, fileLoad)
tStartExp = GetSecs;
...
%         mae_left_y_vali = sum(absoluteError_yvali_left) / validation.nPointValidation;
    ...
    correctBeep = MakeBeep(freqCorrect, responseBeepLengthSecs);
    incorrectBeep = MakeBeep(freqError, responseBeepLengthSecs);
...
%                         eyeAcceleration(eyepos_SN) = velocityComputeOnline(eyeVelocity(2, eyepos_SN-4:eyepos_SN)');
                        ...
                        if counterFixationInitial == params.nFixationSampleInitial
                            ...
                            elseif GetSecs - tStartStim > params.lineDuration
                                ...
                                if keyIsDown && ismember(find(keyCode), [KbName('LeftArrow'), KbName('RightArrow')])
                                    ...
                                    if angleFirstLine == params.lineOrientation(1)
                                        if subjResponse == 1
                                            PsychPortAudio('FillBuffer', pahandleResponse, [correctBeep; correctBeep]);
                                            PsychPortAudio('Start', pahandleResponse, repetitions, startCue, waitForDeviceStart);
                                            nCorrectTrials = nCorrectTrials + 1;
                                        else
                                            PsychPortAudio('FillBuffer', pahandleResponse, [incorrectBeep; incorrectBeep]);
                                            PsychPortAudio('Start', pahandleResponse, repetitions, startCue, waitForDeviceStart);
                                        end                                            
                                    else
                                        if subjResponse == 2
    ...
    Screen('FillRect', windowDisplay, params.backgroundRGB);
    Screen('TextSize', windowDisplay, params.sizeTextPrompt);
    textDisplay = ['Percent correct is: ' num2str(nCorrectTrials/nTrials)];
    DrawFormattedText(windowDisplay, textDisplay, 'center', screenCenter(2)-params.yPositionPromptText, params.textColor);
    Screen('Flip', windowDisplay);  
    KbStrokeWait   
