function plot_model_data(pthetahGtheta_1, pthetahGtheta_2, pProject_1, pProject_2, theta, axis_project, estimate_cond3_1, estimate_cond3_2, estimate_cond3_3, estimate1_allSubject, estimate2_allSubject, ...
                        theta_stim, std_sensory, std_memory_1line, std_memory_2line, std_memory_redraw_1, std_memory_redraw_2, std_memory_redraw_3, ...
                        markerSize, markerColor, dotMeanSize, lineWidth, n_point_kdensity, smooth_sampling, edge_bin, h_filter, n_contour, n_bins, subjectID)
                    
n_trial_model = size(estimate_cond3_1, 2);    
n_trial_data = size(estimate1_allSubject, 2);    

% Get 2d histogram of model for contour plot
[~, ~, est_hist_cond3_1, est_hist_cond3_2, est_hist_cond3_3] = ...
        make_2dhistogram([], [], estimate_cond3_1, estimate_cond3_2, estimate_cond3_3, smooth_sampling, edge_bin, h_filter);    
...
pProject_1 = pProject_1 / diff(axis_project(1:2));
plot(axis_project, pProject_1, 'r', 'LineWidth', lineWidth)
...
histogram(diffEst_data, n_bins, 'Normalization', 'pdf')
pProject_2 = pProject_2 / diff(axis_project(1:2));
plot(axis_project, pProject_2, 'r', 'LineWidth', lineWidth)
xlabel('Orientation difference (deg)')
ylabel('Frequency of occurence')
xlim([min(-2, min(diffEst_data)-1) max(diffEst_data)+1])
axis square
