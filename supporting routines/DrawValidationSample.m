function [Data] = DrawValidationSample(screenCenter, fixation, validation, windowDisplay, textColor, textFontSize)
    real_data = WiseData('get');
    isOnline = real_data(1,1);
    last_SN = real_data(3,1);
    
    numDataPoint = (validation.nPointValidation + 1) * validation.timeValiPoint * 1000 + 100;  
    ...
    Screen('DrawDots', windowDisplay, [screenCenter(1) screenCenter(2)], fixation.sizePixel, fixation.clr, [], 2);
    Screen(windowDisplay, 'Flip');
    WaitSecs(validation.timeValiPoint)

    if isOnline == 1                      % start trigger(do not change here)  
       for i_trial = 1 : validation.nPointValidation                  
            eyepos_SN = 1;
            fixation.Loc_X = screenCenter(1);
            fixation.Loc_Y = screenCenter(2);
                        
            % Present the valibration points
            Screen('DrawDots', windowDisplay, [validation.xValidation(i_trial) validation.yValidation(i_trial)],...
                        fixation.sizePixel, fixation.clr, [], 2);
            Screen(windowDisplay, 'Flip'); 
            
            % Start recording eye data
            tStartValiPoint = GetSecs;
            counter = 1;
            while GetSecs - tStartValiPoint < validation.timeValiPoint               
                real_data = WiseData('get');
        ...
        DrawFormattedText(windowDisplay, 'Validation done!', 'center', 'center', textColor);
        Screen(windowDisplay,'Flip');
        WaitSecs(1); 
    end
end